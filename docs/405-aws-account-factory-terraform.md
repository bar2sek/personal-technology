...
3. **Pure GitOps Workflow**: Simply add a new `aws_organizations_account` block to `bootstrap/aws_organization/main.tf` in Git, create a Pull Request, and merge! ARC automatically provisions the new AWS Account!
...
jobs:
  terraform:
    ...
    steps:
      ...
      - name: Terraform Init
        run: terraform init
        working-directory: ./bootstrap/aws_organization
