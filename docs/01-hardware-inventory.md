...
| `pc-node-04` | AMD Ryzen 7 3800X PC | Ryzen 7 3800X / 64GB RAM / 27.0TB HDD Array (11 HDDs) + 490GB SATA SSDs | Intel Dual 10G SFP+ + Dual 1GbE NIC + Onboard 1GbE (5 Ports) | 80GB Intel 320 SSD (OS) | Storage & Compute Worker |
| `pc-node-05` | Mini-ITX PC (B650I EDGE) | Ryzen 5 7600 (6C/12T) / 32GB RAM / 2x 1TB NVMe SSD / NVIDIA RTX 4070 | 2.5GbE SFP+ + 1GbE USB Adapter | 1x NVMe (OS) / 1x NVMe (Data) | GPU Worker (AI/ML & Transcoding) |
...
3. **Storage & Networking**:
   - **Boot Drives**: Dedicated **16GB SATA SuperDOMs** (single SuperDOMs on `sm-node-01`/`02`, mirrored dual SuperDOMs on `sm-node-03`) provide resilient boot drives for Supermicro nodes. `pc-node-04` boots off a dedicated **80GB Intel 320 Enterprise SATA SSD** (featuring hardware power-loss protection), and `pc-node-05` boots off one of its 1TB NVMe drives.
